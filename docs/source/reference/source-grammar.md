...
Every accepted form of the `source` field, plus the rules for selecting files, naming targets,
...
Recognized when the expression has no scheme, does not begin with `.`, `/`, or `~`, and contains
at least one `/`.
Expanded to a GitHub Git URL.
This is the feature that makes a plain Git repository read like a registry.
...
The fragment carries `ref`, or `ref:path`.
Do not combine an inline fragment path with the `repository:path` compact form,
and do not combine an inline `#ref` with a separate `ref` key.
...
Branch or tag name
: `main`, `v1.2.0`.
  A name that could be either resolves to `reference_kind: ambiguous` and is treated
  conservatively.

Commit SHA
: 7 to 64 hexadecimal characters.
  Fully pinned.
...
Short commit prefix
: Accepted as a requested Git revision, but not sufficient to make a remote lock entry complete
  for frozen replay.
  Frozen replay requires the full 40-character commit captured after checkout.
...
| `>1.2.0` | Exclusive lower bound | tags after `1.2.0` |
| `<=1.2.0` | Inclusive upper bound | tags at or before `1.2.0` |
| `1.2.3` | Exact | only `1.2.3` |

A leading `v` on tags is tolerated: `^1.0.0` matches a tag named `v1.4.2`.

A range is a **moving** reference.
`guidelines update` refreshes it when a newer matching tag appears, but never crosses the
declared bound.
Ranges require the source to publish tags: verify with `git ls-remote --tags <url>`.
...
`path` selects one literal source-relative file or folder.
`paths` selects one or more literal source-relative paths and cannot be combined with `path` or
`pattern`.
`pattern` matches a filename or suffix-stripped filename with a case-sensitive filename glob.

Synchronization always writes the canonical `.guidelines.md` suffix, regardless of the source
filename.
Singular/plural collisions resolve deterministically, preferring the plural source when both
suffixes map to the same destination.

Optional YAML frontmatter is display and search metadata only.
It is never required and never needs provider-specific fields.

Recognized frontmatter keys:

`name`
: Human-readable name.
  Conventionally includes a version number.
...
`metadata`
: Free-form mapping.
  `owner`, `keywords`, and a stable `guideline-id` UUID are conventional.

## Registries

A **registry** is just a Git repository where you store guidelines.
There is no index file, no catalog, and no registry abstraction in the tool: everything resolves
through Git.
...
Tag your releases.
Without tags, consumers can only reference a branch, which is a moving reference: see
{doc}`../how-to/pin-a-version`.
...
Every target is project-relative and must remain inside the project.
Source folders are flattened according to the declaration.

## Reference resolution and caching

Remote sources require Git.
Cache identity excludes credentials and query data.

`--frozen`
: Strict replay.
  No resolution, no acquisition, no cache publication, and no writes.
  Fails if the lock entry, full remote commit, cached snapshot, selected files, or content hashes
...
`--refresh`
: Revalidates discovery against the remote rather than trusting a fresh discovery-cache entry.
  The option is available on `guidelines search`.

`--no-cache`
: Bypasses discovery-cache reads and writes for `guidelines search`.

Exact tags and SHAs remain pinned.
Provider reference metadata takes precedence over name-based guesses.
...
Source and target symlink escapes are rejected.
All destinations and cross-source collisions are validated **before** any mutation.
